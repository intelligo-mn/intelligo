...
const section_entity_1 = __importDefault(require("../../domain/section.entity"));
...
let SectionController = class SectionController {
    constructor(sectionService) {
        ...
        this.logger = new common_1.Logger('SectionController');
    }
    async getAll(req) {
    ...
    async post(req, section) {
        const created = await this.sectionService.save(section);
        header_util_1.HeaderUtil.addEntityCreatedHeaders(req.res, 'Section', created.id);
        return created;
    }
    async put(req, section) {
        header_util_1.HeaderUtil.addEntityCreatedHeaders(req.res, 'Section', section.id);
        return await this.sectionService.update(section);
    }
    async remove(req, id) {
        header_util_1.HeaderUtil.addEntityDeletedHeaders(req.res, 'Section', id);
...
__decorate([
    common_1.Post('/'),
    security_1.Roles(security_1.RoleType.USER),
    swagger_1.ApiOperation({ title: 'Create section' }),
    swagger_1.ApiResponse({
        status: 201,
    ...
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object, section_entity_1.default]),
    __metadata("design:returntype", Promise)
], SectionController.prototype, "post", null);
__decorate([
    common_1.Put('/'),
    security_1.Roles(security_1.RoleType.USER),
    swagger_1.ApiOperation({ title: 'Update section' }),
    swagger_1.ApiResponse({
        status: 200,
...
__decorate([
    common_1.Delete('/:id'),
    security_1.Roles(security_1.RoleType.USER),
    swagger_1.ApiOperation({ title: 'Delete section' }),
    swagger_1.ApiResponse({
        status: 204,
...
SectionController = __decorate([
    common_1.Controller('api/sections'),
