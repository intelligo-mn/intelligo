...
sinon.stub(Intelligo.SlackBot.prototype, 'login');

describe('Intelligo Slack Bot Tests', function() {
  beforeEach(function() {
    this.bot = new Intelligo.SlackBot({ token: 'token' });
  });

  describe('Preprocessing of params', function() {
    it('1', function() {
      var input = {
        foo: 1,
        bar: '2',
  ...
  describe('#api', function() {
    ...
    it('check url', function(done) {
      ...
      this.bot.api('method', { foo: 1, bar: 2, baz: 3 }).then(function() {
    ...
    it('response without error', function(done) {
      sinon.stub(request, 'post').callsFake((data, cb) => {
        cb(null, null, '{"ok": true}');
      });

      this.bot.api('method', { foo: 1, bar: 2, baz: 3 }).then(data => {
    ...
    it('response with error', function(done) {
      ...
      this.bot.api('method').then(function(data) {
  ...
  describe('#postTo', function() {
    ...
    it('1', function(cb) {
      this.bot.getChannels.returns(Promise.resolve({ channels: [{ name: 'name1', is_channel: true }] }));
      this.bot.getUsers.returns(Promise.resolve({ members: [] }));
      this.bot.getGroups.returns(Promise.resolve({ groups: [] }));
      sinon.stub(this.bot, 'postMessageToChannel').returns(Promise.resolve());
    ...
    it('2', function(cb) {
      this.bot.getChannels.returns(Promise.resolve({ channels: [] }));
      this.bot.getUsers.returns(Promise.resolve({ members: [{ name: 'name1' }] }));
      this.bot.getGroups.returns(Promise.resolve({ groups: [] }));
      sinon.stub(this.bot, 'postMessageToUser').returns(Promise.resolve());
    ...
    it('3', function(cb) {
      this.bot.getChannels.returns(Promise.resolve({ channels: [] }));
      this.bot.getUsers.returns(Promise.resolve({ members: [] }));
      this.bot.getGroups.returns(Promise.resolve({ groups: [{ name: 'name1', is_group: true }] }));
      sinon.stub(this.bot, 'postMessageToGroup').returns(Promise.resolve());
  ...
  describe('#getXyzById', function() {
    ...
    it('Channel', function(cb) {
      this.bot.getChannels.returns(
        Promise.resolve({ channels: [{ name: 'name1', id: 'C12345678', is_channel: true }] }),
      );
    ...
    it('User', function(cb) {
      this.bot.getUsers.returns(Promise.resolve({ members: [{ name: 'name1', id: 'U12345678' }] }));
    ...
    it('Group', function(cb) {
      this.bot.getGroups.returns(Promise.resolve({ groups: [{ name: 'name1', id: 'G12345678', is_group: true }] }));
