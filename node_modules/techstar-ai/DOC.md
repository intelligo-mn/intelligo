...
Techstar AI is a machine-learning framework for Node.js. It supports **multi-label classification**, **online learning**, and **real-time classification**. Therefore, it is especially suited for natural language understanding in dialog systems and chat-bots.

Techstar AI is in an "alpha" state - some parts are working (see this readme), but some parts are missing or not tested. Contributions are welcome. 

Techstar AI currently runs on Node.js 0.12 and later versions.

## Installation

	npm install techstar-ai

## Demos

You can run the demos from this project: [Techstar AI-demo](https://github.com/techstar-inc/techstar-bot).
...
```js
var TechstarAI = require('techstar-ai');

var colorClassifier = new TechstarAI.classifiers.NeuralNetwork();
...
### Online Learning
```js
var birdClassifier = new TechstarAI.classifiers.Winnow({
...
```js
var MyWinnow = TechstarAI.classifiers.Winnow.bind(0, {
	default_positive_weight: 1,
	default_negative_weight: 1,
	threshold: 0
});
...
```js
var colorClassifier = new TechstarAI.classifiers.Bayesian();
...
```js
var MyWinnow = TechstarAI.classifiers.Winnow.bind(0, {retrain_count: 10});

var intentClassifier = new TechstarAI.classifiers.multilabel.BinaryRelevance({
...
var intentClassifier = new TechstarAI.classifiers.EnhancedClassifier({
...
Instead of defining your own feature extractor, you can use those already bundled with Techstar AI:

```js
TechstarAI.features.NGramsOfWords
TechstarAI.features.NGramsOfLetters
TechstarAI.features.HypernymExtractor
```
...
intentClassifier = new TechstarAI.classifiers.EnhancedClassifier({
	classifierType: TextClassifier,  // same as in previous example
	normalizer: TechstarAI.features.LowerCaseNormalizer,
...
var TextClassifier = TechstarAI.classifiers.multilabel.BinaryRelevance.bind(0, {
	binaryClassifierType: TechstarAI.classifiers.SvmJs.bind(0, {C: 1.0})
});

// Initialize a classifier with a feature extractor and a lookup table:
var intentClassifier = new TechstarAI.classifiers.EnhancedClassifier({
	classifierType: TextClassifier,
	featureExtractor: TechstarAI.features.NGramsOfWords(1),  // each word ("1-gram") is a feature  
	featureLookupTable: new TechstarAI.features.FeatureLookupTable()
});
...
function newClassifierFunction() {
	...
	return new TechstarAI.classifiers.EnhancedClassifier({
		classifierType: TextClassifier,
		featureExtractor: WordExtractor,
...
var IntentClassifier = TechstarAI.classifiers.EnhancedClassifier.bind(0, {
	classifierType: TechstarAI.classifiers.multilabel.BinaryRelevance.bind(0, {
		binaryClassifierType: TechstarAI.classifiers.Winnow.bind(0, {retrain_count: 10})
	}),
	featureExtractor: TechstarAI.features.NGramsOfWords(1),
});

var microAverage = new TechstarAI.utils.PrecisionRecall();
var macroAverage = new TechstarAI.utils.PrecisionRecall();

Techstar AI.utils.partitions.partitions(dataset, numOfFolds, function(trainSet, testSet) {
	console.log("Training on "+trainSet.length+" samples, testing on "+testSet.length+" samples");
	var classifier = new IntentClassifier();
	classifier.trainBatch(trainSet);
	Techstar AI.utils.test(classifier, testSet, /* verbosity = */0,
...
The Techstar AI.js package provides wrappers for these implementations. 
...
var intentClassifier = new TechstarAI.classifiers.EnhancedClassifier({
	classifierType: TechstarAI.classifiers.multilabel.BinaryRelevance.bind(0, {
		binaryClassifierType: TechstarAI.classifiers.SvmPerf.bind(0, 	{
			learn_args: "-c 20.0" 
		})
	}),
	featureExtractor: TechstarAI.features.NGramsOfWords(1),
...
Do you like Techstar AI? Remember that you can star it :-)
