...
const chatModels = require("../models/intelligo-chat.models");
...
let SimulatorService = class SimulatorService {
        ...
        if (message.type == chatModels.MessageType.INPUT) {
            ...
            if (ipMsgContent.input && Object.keys(ipMsgContent.input).length > 0) {
                ...
                switch (ipMsgContent.inputType) {
                    case chatModels.InputType.LOCATION:
                        {
                        ...
                        break;
                    case chatModels.InputType.DATE:
                        {
                        ...
                        break;
                    case chatModels.InputType.TIME:
                        {
                            let ip = ipMsgContent.input;
                            userData = ip.time;
                        ...
                        break;
                    case chatModels.InputType.ADDRESS:
                        {
                            let ip = ipMsgContent.input;
                            userData = ip.address;
                        ...
                        break;
                    case chatModels.InputType.MEDIA:
                        {
                            let ip = ipMsgContent.input;
                        ...
                        break;
                    case chatModels.InputType.OPTIONS:
                        {
                            let ip = ipMsgContent.input;
                        ...
                        break;
                    case chatModels.InputType.LIST:
                        {
    ...
    replaceTxt(subStr, key) {
        ...
        try {
            if (this.state.variables && this.state.variables[key])
                ...
                let deepValue = jsonpath.query(wrappedResp, key);
    ...
    processNode(chatNode, section) {
        ...
        switch (chatNode.NodeType) {
            case models.NodeType.ApiCall:
                {
                    let apiHeaders = new http_1.HttpHeaders();
                    if (chatNode.Headers) {
                    ...
                    let reqParams = new http_1.HttpParams();
    ...
    convertButtons(chatNode, callback) {
        ...
        if (clickInputs && clickInputs.length > 0) {
            if (_.filter(clickInputs, x => _.contains([models.ButtonType.NextNode, models.ButtonType.OpenUrl], x.ButtonType)).length > 0) {
                let optionsInput = {
                    inputType: chatModels.InputType.OPTIONS,
                    mandatory: mandatory,
        ...
        try {
            if (chatNode.Buttons) {
                for (var btnIdx = 0; btnIdx < chatNode.Buttons.length; btnIdx++) {
                    ...
                    let wrappedResp = {};
                    wrappedResp[rootToken] = JSON.parse(this.state.variables[rootToken]);
                    let deepValue = jsonpath.query(wrappedResp, btn.ConditionMatchKey);
                    if (deepValue && typeof deepValue == 'object' && deepValue.length == 1) {
                        deepValue = deepValue[0];
                    }
...
SimulatorService = __decorate([
    core_1.Injectable(),
    __metadata("design:paramtypes", [http_1.HttpClient, globals_service_1.GlobalsService, info_dialog_service_1.InfoDialogService])
], SimulatorService);
