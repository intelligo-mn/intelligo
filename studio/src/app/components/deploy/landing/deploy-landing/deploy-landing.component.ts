...
export class DeployLandingComponent implements OnInit {
  ...
  loadSavedOptions() {
    let key = Md5.hashStr(
      `${JSON.stringify(this.dataService.chatServer.ServerUrl)}|${
        this.webOptions.businessId
      }|${this.webOptions.flowId}`,
    ) as string;
...
src="${this.n(
      this.webOptions.websdkUrl,
    )}assets/embed/intelligo-web-chat-plugin.js" 
intelligo-api-endpoint="${this.n(this.dataService.chatServer).ServerUrl}"
intelligo-endpoint="${this.n(
      this.webOptions.webSocketsUrl,
    )}/wscustomers/chatcustomers-websocket"
...
${this.webOptions.isFullPage ? '\nintelligo-fullpage="true"' : ''} ${
      this.webOptions.allowChatReset
        ? '\nintelligo-allow-chat-reset="true"'
        : ''
    } ${
      this.webOptions.enableHtmlMessages
        ? '\nintelligo-html-messages="true"'
        : ''
    } ${
      this.webOptions.showPoweredByAna ? '\nintelligo-show-branding="true"' : ''
    } ${
      this.webOptions.autoOpenSecs
