...
export class ChatFlowComponent implements OnInit, OnDestroy {
  ...
  updateNodeLayout(chatNodeVM: ChatNodeVM): boolean {
    ...
    if (btnsTable) {
      ...
      else chatNodeVM._btnTableWidth = btnsTable.clientWidth;
  ...
  mouseDown(event: MouseEvent) {
    ...
    if (
      this.chatFlowNetwork.newChatNodeConnection.isHidden &&
      !this.chatFlowNetwork.draggingChatNode
    )
      this._isMouseDown = true;
    else this._isMouseDown = false;
  }
  ...
  zoomToRectAnimIntermediate(
    ...
    if (
      !this.approxEquals(this._viewBoxX, x2, step) ||
  ...
  fitViewToNodes(chatNodeVMs: ChatNodeVM[]) {
    ...
    if (width < Config.maxZoomWidth) width = Config.maxZoomWidth;
    if (height < Config.maxZoomHeight) height = Config.maxZoomHeight;
  ...
  designerWheel(event: WheelEvent) {
    ...
    if (this._viewBoxWidth - change > 0) this._viewBoxWidth -= change;

    if (this._viewBoxHeight - change > 0) this._viewBoxHeight -= change;
  ...
  layoutReady() {
    if (!this.chatFlowNetwork.chatNodeVMs) return true;
    else
      return (
        this.chatFlowNetwork.chatNodeVMs.filter(x => x._layoutUpdated).length ==
        this.chatFlowNetwork.chatNodeVMs.length
      );
  }

  initialZoom() {
    if (this.layoutReady()) this.fitViewToAllNodes();
    else setTimeout(() => this.initialZoom(), 500);
  }
...
class ChatNodeNewConnection {
  ...
  srcConnectorX() {
    if (this.srcButtonConnector) return this.srcButtonConnector.x();
    return 0;
  }
  srcConnectorY() {
    if (this.srcButtonConnector) return this.srcButtonConnector.y();
    return 0;
  }
