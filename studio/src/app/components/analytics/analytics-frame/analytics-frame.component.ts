    ...
    if (this.appHeader) {
      this.appHeader.goBack = () => {
        this.infoDialog.confirm(
          ...
          'Are you sure you want to exit from analytics and go back to home page?',
          ok => {
            if (ok) {
  ...
  constructor(
    ...
    this.route.queryParams.subscribe(x => {
      ...
      if (
        ...
        let initialUrl = `/index.html#/?apiBase=${encodeURIComponent(
          x['apiBase'],
        )}&businessId=${x['businessId']}&businessName=${encodeURIComponent(
          x['businessName'],
        )}&chatFlowId=${x['chatFlowId']}`;
        let url = 'analytics-dashboard' + initialUrl;

        if (environment.local) url = 'http://localhost:4202' + initialUrl;
