...
export class BusinessPickerComponent implements OnInit {
      ...
      (x) => {
        ...
        if (x.success) {
          ...
          this.dataService.handleTypedError(
            ...
            "Something went wrong while loading business account. Please try again."
          );
        }
      },
      (err) => {
  ...
  bizFilteredOptions() {
    if (typeof this.bizFilter === "string") {
      ...
      if (this.bizFilter) {
        list = this.businessAccounts.filter(
          (x) =>
            x.name
              .toLowerCase()
              .indexOf((<string>this.bizFilter).toLowerCase()) != -1 ||
            x.id
              .toLowerCase()
              .indexOf((<string>this.bizFilter).toLowerCase()) != -1
        );
      ...
      return list.sort((x, y) =>
        x.name && y.name ? x.name.localeCompare(y.name) : 1
      );
    }
  }
  ...
  projFilteredOptions() {
    if (typeof this.projFilter === "string") {
      ...
      if (this.projFilter) {
        list = this.chatProjects.filter(
          (x) =>
            x.name
              .toLowerCase()
              .indexOf((<string>this.projFilter).toLowerCase()) != -1 ||
            x.id
              .toLowerCase()
              .indexOf((<string>this.projFilter).toLowerCase()) != -1
        );
  ...
  loadChatProjects() {
    if (!this.param.askFlowId) return false;
    if (!this.selectedBusinessAccount) return false;

    this.infoDialog.showSpinner();
    this.dataService
      .getChatProjects(this.selectedBusinessAccount.id, 0, 10000)
      .subscribe(
        (x) => {
          this.infoDialog.hideSpinner();
          if (x.success) {
            ...
            this.dataService.handleTypedError(
              x.error,
              "Unable to fetch chat projects",
              "Something went wrong while trying to fetch chat projects. Please try again."
            );
          }
        },
        (err) => {
