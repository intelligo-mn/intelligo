...
@Component({
  selector: 'app-cloud-signup',
  templateUrl: './cloud-signup.component.html',
  styleUrls: ['./cloud-signup.component.scss'],
})
export class CloudSignupComponent implements OnInit {
  constructor(
  ...
  allValid(): boolean {
    ...
    if (!d.email || !this.global.emailValid(d.email)) {
      this.errorMessage =
        'Please enter a valid email address. Verification email will be sent to it.';
      return false;
    }
  ...
  create() {
    if (this.allValid()) {
      ...
      this.data.getRoles().subscribe(
        x => {
          let roles = x.data.filter(
            x => ['BUSINESS_ADMIN'].indexOf(x.role) != -1,
          );
            ...
            this.data
              .registerOnAnaCloud({
                business: {
                  ...
                  name:
                    this.details.companyName + ' - ' + this.details.displayName,
                },
                user: {
                  email: this.details.email,
                  phone: this.details.phone,
