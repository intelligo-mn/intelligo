    ...
    this.loginService.performLogin(true, '/', true, done => {
      ...
      if (this.dataService.loggedInUser) {
        if (this.dataService.isSuperAdmin()) {
          ...
          this.dataService.isBizAdmin() &&
          this.dataService.loggedInUser.businessId
        ) {
          this.router.navigateByUrl(
            '/manage-users/users?bizId=' +
              this.dataService.loggedInUser.businessId,
          );
        } else {
          this.infoDialog.alert(
            'Unauthorized!',
            'Only a super admin or a business admin can login into user management',
          );
        }
      }
    });
    ...
    this.loginService.performLogin(true, null, true, done => {
      ...
      if (this.dataService.loggedInUser) {
        if (this.dataService.isSuperAdmin()) {
          ...
          (this.dataService.isBizAdmin() || this.dataService.isFlowManager()) &&
          this.dataService.loggedInUser.businessId
        ) {
          ...
          this.infoDialog.alert(
            'Unauthorized!',
            'Only a super admin, a business admin or a flow manager can login into user management',
          );
        }
      }
    });
    ...
    d.afterClosed().subscribe((x: ChoosenBizAccChatProj) => {
      if (x && x.bizAccount && x.chatProj) {
        this.infoDialog.prompt(
          ...
          'Please enter the analytics server url',
          result => {
            if (result) {
