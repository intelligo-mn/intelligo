...
export class SimulatorService {
  ...
  public simulatorBusinessId = 'intelligo-studio';
  public simulatorCustomerId = 'intelligo-simulator';
  ...
  private processIncomingMessage(chatMsg: chatModels.IntelligoChatMessage) {
    ...
    if (message.type == chatModels.MessageType.INPUT) {
          ...
          case chatModels.InputType.MEDIA: //Click, Non Complex
            {
              ...
              if (ip.media && ip.media.length > 0 && ip.media[0]) {
                if (typeof ip.media[0].url == 'object')
                  userData = (<any>ip.media[0].url)
                    .changingThisBreaksApplicationSecurity;
                else userData = ip.media[0].url;
          ...
          case chatModels.InputType.LIST: //Click, Complex
            {
              ...
              let listSelectedItems = ipMsg.values.filter(
                x => listSelectedValues.indexOf(x.value) != -1,
              );
      ...
      if (
        msgContent.input &&
        Object.keys(msgContent.input).indexOf('val') != -1 &&
        msgContent.input.val
      ) {
  ...
  private gotoNextNode(nextNodeId: string) {
    ...
    if (nextNode) this.processNode(nextNode);
  }
  ...
  private replaceTxt(subStr, key) {
    if (!key) key = subStr.replace('{{', '').replace('}}', '');
      ...
      case models.NodeType.ApiCall:
        {
          ...
          this.http
            ...
            .subscribe(
              ...
              err => {
                if (
                  Math.trunc(err.status / 100) == 5 ||
                  typeof err._body == 'object'
                ) {
                  //Only 5xx errors are counted as errors
                  this.logDebug(err);
  ...
  ): chatModels.IntelligoMessageData {
  ...
  private convertButtons(
    ...
    if (clickInputs && clickInputs.length > 0) {
      if (
        ...
        //If next node/open url button is present, only options can be sent
  ...
  private convertButtonTypeToInputType(
    srcType: models.ButtonType,
  ): chatModels.InputType {
    switch (srcType) {
      default:
  ...
  private convertCarouselButtonType(
    srcType: models.CarouselButtonType,
  ): chatModels.ButtonType {
    switch (srcType) {
      case models.CarouselButtonType.DeepLink:
      case models.CarouselButtonType.OpenUrl:
