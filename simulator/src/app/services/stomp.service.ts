...
export class StompService {
  ...
  public connect(config?: StompConfig) {
    ...
    if (!this.client) throw Error("Client not configured!");
  ...
  private subscribe = () => {
    ...
    let channel = this.config.flowId
      ? "/topic/chat/customer/" +
        custId +
        "/business/" +
  ...
  private onError = (error: string | StompJS.Message) => {
    ...
    if (typeof error === "object") error = error.body;

    if (this.config && this.config.debug)
      this.debug("Socket Error: " + JSON.stringify(error));

    this.debug(`Error: ${error}`);
    if (error.indexOf("Lost connection") !== -1) this.delayReconnect(5000);
  };
  ...
  private onAck = (msgAckId: string, delivered?: boolean) => {
    ...
    if (this.handleAck) this.handleAck(msgAckId, delivered);
  };

  private onTyping = () => {
    this.debug("Typing... ");
    if (this.handleTyping) this.handleTyping();
  };
    ...
    if (anaMsg.data && Object.keys(anaMsg.data).length > 0) {
      if (this.handleMessageReceived) {
        if (this.msgsIds.indexOf(anaMsg.meta.id) == -1) {
          //handle message only if it is not already handled
