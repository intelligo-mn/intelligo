...
export class UtilitiesService {
  ...
  static anaTimeToDate(anaTime: IntelligoTime) {
    ...
    d.setHours(
      ...
      parseInt(anaTime.minute),
      parseInt(anaTime.second)
    );
    return d;
  }
  ...
  static googleMapsStaticLink(latLng: GeoLoc) {
    return `https://maps.googleapis.com/maps/api/staticmap?center=${
      ...
      latLng.lng
    }&zoom=13&size=300x150&maptype=roadmap&markers=color:red|label:A|${
      latLng.lat
    },${latLng.lng}&key=${UtilitiesService.googleMapsConfigRef.apiKey}`;
  }

  static getAnaMediaTypeFromMIMEType(mimeType: string) {
    let assetType: MediaType;
    if (mimeType.startsWith("image")) assetType = MediaType.IMAGE;
    else if (mimeType.startsWith("video")) assetType = MediaType.VIDEO;
    else if (mimeType.startsWith("audio")) assetType = MediaType.AUDIO;
    else assetType = MediaType.FILE;
    return assetType;
  }
...
export class Config {
  ...
  static consecutiveErrorsMessageText =
    "Uh oh, seems like you've lost your internet connection";
